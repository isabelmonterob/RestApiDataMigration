...
def read_departments(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud_service.get_departments(db, skip=skip, limit=limit)

@router.get("/{department_id}", response_model=schemas.Departaments)
def read_department(department_id: int, db: Session = Depends(get_db)):
    data = crud_service.get_department(db, department_id=department_id)
    if data is None:
...
def delete_departments(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud_service.delete_departments(db, skip=skip)